...
function processLastItem(stringList, callback) {
 let lastEl = stringList[stringList.length - 1];
 return callback(lastEl);
}
...
function lowerCaseStrings(strings) {
  let lStrings = [];
  strings.forEach(item => lStrings.push(item.toLowerCase()));
  return lStrings;
}
...
function isItAnApple(strings) {
  let trueFalse = strings.map(item => {
    if (item === 'apple') {
      return true;
    }
    else {
...
function removeApple(strings) {
  let filtered = strings.filter(item => item !== 'apple');
  return filtered;
...
function stringSmash(strings) {
  let smashed = strings.reduce((accumulator, currentItem) => {
